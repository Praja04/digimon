...
class BlendingAwalController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $ggaBatches = BlendingAwalModel::where('production_batch_id', $request->production_batch_id)->get();
        foreach ($ggaBatches as $record) {
            [$existStart, $existEnd] = explode('-', $record->batch_range);
            if ($start <= (int)$existEnd && $end >= (int)$existStart) {
                $existingBatches->push("Blending Awal ($existStart - $existEnd)");
            }
        }
    ...
    public function updateAjaxBlending(Request $request, $id)
    {
        ...
        if ($disposition === 'Resampling') {
            $dataUpdate['disposition_remarks'] = $remarks ? $remarks . ' (Resampling)' : 'Resampling';
            $dataUpdate['not_standar'] = true;
        }
