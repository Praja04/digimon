...
class BlendingAfterAdjustController extends Controller
{
    //
    //
    public function analisaBlendingAfter(Request $request)
    {
        ...
        $rawData = BlendingAfterAdjustModel::with(['productionBatch:id,po_number,variant'])
            ->select(
                'batch_range',
                'nomor_blending',
                'brix',
    ...
    public function analisaDisposisi(Request $request)
    {
        ...
        $dispositions = BlendingAfterAdjustModel::whereNotNull('disposition')
            ->whereBetween('created_at', [$startDate, $endDate])
            ->get()
            ->groupBy('disposition')
