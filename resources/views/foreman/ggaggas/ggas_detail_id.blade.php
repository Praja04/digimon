        ...
        <div class="card">
            ...
            <div class="card-body">
                <form class="ajax-ggas-form" data-id="{{ $ggas->id }}">
                    ...
                    <div class="mb-3">
                        <label for="nacl" class="form-label">NACL Optional</label>
                        <input type="number" step="0.01" max="100" min="0" name="nacl" id="nacl" class="form-control" value="{{ old('nacl', $ggas->nacl) }}">
                    </div>

                    <div class="mb-3">
                        <label for="warna" class="form-label">Warna</label>
                        <input type="text" name="warna" id="warna" class="form-control" required value="{{ old('warna', $ggas->warna) }}" oninput="this.value = this.value.toUpperCase();">
...
<script>
    $(document).ready(function() {
        ...
        $('.ajax-ggas-form').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ url('foreman/ggaggas/ggas/update-ajax') }}/" + id,
                method: 'POST',
                data: form.serialize(),
                success: function(response) {
                    Swal.fire({
                        ...
                        window.location.href = "{{ url('foreman/ggaggas/ggas') }}/" + url;
                    });
                },
                error: function(xhr) {
