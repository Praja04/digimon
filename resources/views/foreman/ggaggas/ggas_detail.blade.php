...
@section('content')
                            ...
                            <tbody class="list form-check-all">
                                @forelse ($productionBatch->GgasProcesses as $ggas)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($ggas->disposition_remarks != null && $ggas->disposition_remarks != '-' && $ggas->disposition != 'Adjustment')
                                                {{ $ggas->disposition_remarks }}
                                            @elseif($ggas->disposition == 'Adjustment')
                                                Adjustment Air: {{ $ggas->adjustment_qty_air }} Liter, Garam:
                                                {{ $ggas->adjustment_qty_garam }} Kg, Gula:
                                                {{ $ggas->adjustment_qty_gula }} Kg
                                            @elseif($ggas->is_adjustment == true)
                                                After Adjustment
                                            @else
                                                -
                                            @endif
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <button type="button" class="btn btn-sm btn-warning open-ggas-modal-edit"
                                                    data-id="{{ $ggas->id }}" data-brix="{{ $ggas->brix }}"
                                                    data-nacl="{{ $ggas->nacl }}" data-warna="{{ $ggas->warna }}"
                                                    data-disposition="{{ $ggas->disposition }}"
                                                    data-remarks="{{ $ggas->disposition_remarks }}"
                                                    data-adjustment-air="{{ $ggas->adjustment_qty_air }}"
                                                    data-adjustment-gula="{{ $ggas->adjustment_qty_gula }}"
                                                    data-adjustment-garam="{{ $ggas->adjustment_qty_garam }}"
                                                    data-bs-toggle="modal" data-bs-target="#editGgasModal">
                                                    Edit GGAS
                                                </button>
                                            @endif
                                ...
                                <div class="modal fade" id="inputGgasModal" tabindex="-1"
                                    aria-labelledby="inputGgasModalLabel" aria-hidden="true">
                                ...
                                <div class="modal fade" id="editGgasModal" tabindex="-1"
                                    aria-labelledby="editGgasModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <form id="ggasFormedit">
                                            @csrf
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title">Edit Data GGAS</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Tutup"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="mb-3">
                                                        <label class="form-label">BRIX</label>
                                                        <input type="number" step="0.01" name="brix_edit"
                                                            class="form-control" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label class="form-label">NACL</label>
                                                        <input type="number" step="0.01" name="nacl_edit"
                                                            class="form-control" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label class="form-label">Warna</label>
                                                    ...
                                                    <div class="mb-3">
                                                        <label class="form-label">Disposition</label>
                                                        <select name="disposition_edit" class="form-select" required>
                                                            <option value="">-- Pilih Disposition --</option>
                                                            <option value="Release">Release</option>
                                                            <option value="Release Bersyarat">Release Bersyarat</option>
                                                            <option value="Resampling">Resampling</option>
                                                    ...
                                                    <div class="mb-3 d-none adjustment-qty-wrapper">
                                                        ...
                                                        <div class="row g-3">
                                                            <div class="col-md-4">
                                                                <label class="form-label">Air (Liter)</label>
                                                                <input type="number" step="0.01"
                                                                    name="adjustment_qty_edit_air" class="form-control">
                                                            </div>
                                                            <div class="col-md-4">
                                                                <label class="form-label">Gula (Kg)</label>
                                                                <input type="number" step="0.01"
                                                                    name="adjustment_qty_edit_gula" class="form-control">
                                                            </div>
                                                            <div class="col-md-4">
                                                                <label class="form-label">Garam (Kg)</label>
                                                                <input type="number" step="0.01"
                                                                    name="adjustment_qty_edit_garam" class="form-control">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.open-ggas-modal-edit', function() {
                ...
                // Set nilai ke form
                $('#editGgasModal input[name="brix_edit"]').val(brix);
                $('#editGgasModal input[name="nacl_edit"]').val(nacl);
                $('#editGgasModal select[name="warna_edit"]').val(warna);
                $('#editGgasModal select[name="disposition_edit"]').val(disposition);
                $('#editGgasModal textarea[name="disposition_remarks_edit"]').val(remarks);

                // Tambahkan ID tersembunyi ke form
                if ($('#ggasFormedit input[name="id"]').length === 0) {
                    $('#ggasFormedit').append('<input type="hidden" name="id" value="' + id + '">');
                } else {
                    $('#ggasFormedit input[name="id"]').val(id);
                }

                // Tampilkan atau sembunyikan kolom adjustment sesuai disposition
                if (disposition === 'Adjustment') {
                    $('.adjustment-qty-wrapper').removeClass('d-none');
                    $('#editGgasModal input[name="adjustment_qty_edit_air"]').val(adjustmentAir);
                    $('#editGgasModal input[name="adjustment_qty_edit_gula"]').val(adjustmentGula);
                    $('#editGgasModal input[name="adjustment_qty_edit_garam"]').val(adjustmentGaram);
                } else {
                    $('.adjustment-qty-wrapper').addClass('d-none');
                    $('#editGgasModal input[name="adjustment_qty_edit_air"]').val('');
                    $('#editGgasModal input[name="adjustment_qty_edit_gula"]').val('');
                    $('#editGgasModal input[name="adjustment_qty_edit_garam"]').val('');
                }

                // Tampilkan modal
                $('#editGgasModal').modal('show');
            });
            ...
            $('#ggasForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ url('/foreman/ggaggas/ggas/update-ajax') }}/" + selectedId,
                    method: 'POST',
                    data: form.serialize(),
            ...
            $('#ggasFormedit').on('submit', function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    url: "{{ url('/foreman/ggaggas/ggas/edit') }}/" + selectedId,
                    method: 'POST',
                    data: form.serialize(),
                    success: function(response) {
                        $('#editGgasModal').modal('hide');
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            text: response.message || 'Data berhasil disimpan.'
