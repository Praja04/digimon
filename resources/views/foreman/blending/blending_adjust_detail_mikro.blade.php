                ...
                <div class="table-responsive table-card mb-4">
                    <table class="table align-middle table-nowrap mb-0 text-center" id="tasksTable">
                        ...
                        <tbody class="list form-check-all">
                            @if ($productionBatch && $productionBatch->blendingAfterAdjustMikro)
                            ...
                            <tr>
                                ...
                                <td>{{ $blending->volume_blending }}</td>
                                <td>{{ $blending->eb }}</td>
                                <td>{{ $blending->tpc }}</td>
                                <td>{{ $blending->ym }}</td>
                                <td>
                                    ...
                                    <button class="btn btn-sm btn-primary open-blending-modal-edit" data-id="{{ $blending->id }}">Edit</button>

                                    @endif
                                </td>
                                <td>{{ $blending->hasil }}</td>
                            </tr>
                            @endforeach
                            @else
                            <tr>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#blendingForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{url('/foreman/blending/mikro/update')}}/" + selectedId,
                method: 'POST',
                data: form.serialize(),
        ...
        $('#blendingFormEdit').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{url('/foreman/blending/mikro/edit')}}/" + selectedId,
                method: 'POST',
                data: form.serialize(),
                success: function(response) {
                    $('#inputBlendingModal').modal('hide');
