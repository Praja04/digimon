...
@section('content')
    ...
    {{-- Fungsi getBadgeClass disalin dari Foreman --}}
    @php
                ...
                <div class="card-header border-0">
                    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                        ...
                        <div class="text-center text-md-start">
                            <a class="btn btn-danger w-100 w-md-auto" href="{{ url('analis/rmpm/list/rm') }}">
                                Kembali
                            </a>
                        </div>
                        ...
                        <div class="d-flex flex-wrap justify-content-center justify-content-md-end gap-2">
                            ...
                            {{-- Tombol Hapus tidak muncul untuk Analis --}}
    ...
    <div class="row justify-content-center">
        <div class="col-xxl-12">
                    ...
                    <div class="col-12">
                        <div class="card shadow-sm">
                            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                                ...
                                {{-- Tombol Update Dihapus dari tampilan Analis --}}
                                {{-- @if ($data_dokumen)
            ...
            'kesesuaian_matriks_bahan' => 'Kesesuaian Matriks Bahan Baku',
                        ...
                        {{-- Modal Edit Dokumen Dihapus dari tampilan Analis --}}
            ...
            'berbau' => 'Berbau',
                        ...
                        {{-- Modal Edit Kondisi Mobil Dihapus dari tampilan Analis --}}
                    ...
                    <div class="col-12">
                        ...
                        {{-- Modal Edit Kemasan Dihapus dari tampilan Analis --}}
            ...
            'sesuai_std' => 'Sesuai Standar',
                            ...
                            {{-- Modal Edit Raw Dihapus dari tampilan Analis --}}
                                            ...
                                            <tbody id="list-detail-long">
                                                @forelse ($analisa_long_term as $index => $long)
                                                    <tr>
                                                        ...
                                                        <td class="text-end">
                                                            {{-- Tombol Update Disposisi hanya untuk Foreman/Role lain yang punya hak akses --}}
    ...
    </div> {{-- MODAL HANYA UNTUK TAMPILAN (BUKAN UNTUK EDIT/UPDATE SEPERTI DI FOREMAN) --}}
                ...
                <div class="modal-body">
                    <div id="formKonfirmasi">
                        <div class="mb-3">
                            ...
                            {{-- Gunakan nilai default waktu saat ini untuk Analis --}}
    ...
    {{-- Script JavaScript disalin dari Foreman dengan penyesuaian: --}}
    {{-- 1. Penghapusan/Penonaktifan fungsi yang hanya ada di Foreman (e.g., deleteMultiple, updateDisposisiLong) --}}
    {{-- 2. Penyesuaian URL/Route dari 'foreman' ke 'analis' di beberapa bagian yang relevan (e.g., Confimation) --}}
    {{-- 3. Mempertahankan logika Form Analisa (modalAnalisa) yang kompleks. --}}
    ...
    <script>
            ...
            samplingButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    setTimeout(() => {
                        ...
                        if (modalId) {
                            // Gunakan vanilla JS untuk bootstrap modal
        ...
        // =====================================
        // SAMPLING FORM HANDLERS (Simpan Data Sampling Awal)
        // =====================================
        $(document).ready(function() {
            ...
            $('form.form-sampling input[type=radio]').on('change', function() {
                ...
                // Logika show/hide sama seperti Foreman
                if (name === 'sesuai_std') {
                    if (value === 'no') {
                        wrapper.show();
                    }
            ...
            function prepareRadioValues(form) {
                form.find('.mb-3').each(function() {
                    ...
                    // Untuk menyamakan, saya akan menggunakan LOGIKA FOREMAN yang lebih RAPI:
                    if (radio.length > 0 && zakInput) {
                        const name = radio.attr('name');
                        if (name === 'sesuai_std' && radio.val() === 'no') {
                            radio.val(`no, karena ${zakInput}`);
                        } else if (name !== 'sesuai_std' && radio.val() === 'yes') {
                            radio.val(`yes, dengan jumlah ${zakInput}`);
                        }
                    } else if (radio.length > 0 && radio.val() === 'no') {
                        radio.val('no');
                    } else if (radio.length > 0 && radio.val() === 'yes') {
                        radio.val('yes');
                    } else if (radio.length === 0) {
                        // Handle unchecked radios (optional, backend validation should handle this)
                    }
            ...
            $('#form-kondisi-mobil').on('submit', function(e) {
                ...
                const form = $(this);
                // prepareRadioValues(form); // Matikan prepare karena form mobil di atas sudah disederhanakan tanpa input zak
            ...
            $('#form-dokumen, #form-kemasan').on('submit', function(e) {
                ...
                const form = $(this);
                prepareRadioValues(form); // Gunakan prepareRadioValues untuk dokumen/kemasan
                let data = form.serialize();
            ...
            $('#form-raw').on('submit', function(e) {
                ...
                const form = $(this);
                prepareRadioValues(form); // Gunakan prepareRadioValues untuk raw
                let data = form.serialize();
        ...
        // =====================================
        $(document).ready(function() {
            var tipeInput = 'kedatangan'; // Default input adalah kedatangan

            $('#btnBukaModalKonfirmasi').click(function() {
                $('#pesanSelesai').remove();
                $('#labelJam, #jamInput, #btnSimpanJam').show();
            ...
            function Konfirmasi() {
                $.ajax({
                    // URL disesuaikan ke konteks Analis
                    ...
                    success: function(response) {
                        ...
                        if (response.sampling_complete && !response.jam_analisa_exists) {
                            // KONDISI 1: Sampling Selesai, Jam Analisa Belum Ada
                            tipeInput = 'analisa';
                            $('#labelJam').text('Jam Analisa');
                            $('#jamInput').val(
                                '{{ now()->format('Y-m-d\TH:i') }}');
                            $('#labelJam, #jamInput, #btnSimpanJam').show();
                            $('#pesanRequiredSampling').hide();

                        } else if (!response.sampling_complete) {
                            // KONDISI 2: Sampling Belum Selesai (atau Belum Ada Jam Kedatangan)
                            ...
                            // KONDISI 3: Sudah diisi semua (Jam Kedatangan DAN Jam Analisa sudah ada)
                            // Sembunyikan form input dan tampilkan pesan selesai
                            $('#labelJam, #jamInput, #btnSimpanJam').hide();
                            $('#formKonfirmasi').append(
                                // Hanya tambahkan satu div pesan selesai
                                '<div class="alert alert-info text-center" id="pesanSelesai">Semua jam telah dikonfirmasi.</div>'
                            );
                        }
                    },
                    error: function(xhr, status, error) {
                        ...
                        $('#formKonfirmasi').append(
                            '<div class="alert alert-danger text-center">Gagal memuat status konfirmasi.</div>'
                            );
                        $('#labelJam, #jamInput, #btnSimpanJam').hide();
                    }
                });
            ...
            $('#btnSimpanJam').click(function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        console.error('Error:', error);
                        Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan jam.', 'error');
                    }
                });
            });

            // Panggil Konfirmasi saat modal terbuka untuk menentukan tipe input
            $('#modalKonfirmasi').on('show.bs.modal', function() {
                Konfirmasi();
            });

            // Bersihkan pesan selesai saat modal tertutup
        ...
        // =====================================
        // ANALISA FORM - MAIN LOGIC (disalin dari Foreman)
        // =====================================
        ...
        function saveDraft() {
            ...
            $('#formAnalisa').find('input, select, textarea').each(function() {
                ...
                if (name && value !== undefined) {
                    if (name.endsWith('[]')) {
                        // Skip if the array input is for file
                        ...
                        if (!draftData[cleanName]) draftData[cleanName] = [];
                        draftData[cleanName].push(value);

                    } else if ($(this).attr('type') === 'file') {
                        // Skip file input for draft data
                        return true;
                    } else {
            ...
            // Clean up old array entries before collecting new ones for better draft handling
            ...
            // Recalculate array entries to avoid duplicates
            $('#form-analisa-content').find('input[name$="[]"], select[name$="[]"], textarea[name$="[]"]').each(
                function() {
        ...
        function loadDraft(draftData = null) {
            ...
            for (const name in draftData) {
                ...
                let elements;
                let isArray = Array.isArray(draftData[name]);
            ...
            setTimeout(() => {
                ...
                if (firstTab.length) {
                    const tabTrigger = new bootstrap.Tab(firstTab[0]);
                    tabTrigger.show();
                }
            }, 200);
