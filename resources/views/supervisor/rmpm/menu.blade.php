...
<div class="row mt-4">
    @foreach ($jenis_gula as $gula)
    <div class="col-xl-3 col-lg-6">
        <div class="card ribbon-box right overflow-hidden">
            <div class="card-body text-center p-4">
                <div class="ribbon ribbon-{{ $gula['ribbon'] }} ribbon-shape trending-ribbon">
                    <i class="ri-hand-heart-fill text-white align-bottom"></i>
                    <span class="trending-ribbon-text">{{ $gula['nama'] }}</span>
                </div>
                <img src="{{ asset('assets/images/' . $gula['image']) }}" alt="" height="45">
                <h5 class="mb-1 mt-4"><a href="{{ route('rmpm.listIdentitas', ['jenis' => $gula['nama']]) }}" class="link-primary">{{ $gula['nama'] }}</a></h5>
                <p class="text-muted mb-4">Identitas RM</p>
                <div class="row justify-content-center">
                    <div class="col-lg-8">
                        <div id="chart-{{ Str::slug($gula['nama']) }}" data-colors='["--vz-danger"]'></div>
                    </div>
                </div>

                <div class="mt-4">
                    <a href="{{ route('rmpm_supervisor.list_data', ['jenis' => $gula['nama']]) }}" class="btn btn-light w-100">Lihat Detail</a>
                </div>
            </div>
        </div>
    </div>
